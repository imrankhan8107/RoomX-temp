...
const index = ({ hotelList }) => {
    ...
    useEffect(() => {
        const getHotelByCity = async () => {
            if (city !== '') {
                const response = await axios.get(`http://localhost:4000/api/orgs?city=${city}`);
    ...
    const handleSubmit = async (e) => {
        ...
        const hotels = await axios.get(
            `http://localhost:4000/api/orgs?city=${city?.toLocaleLowerCase()}&min=${min}&max=${max}`
        );
                            ...
                            <DateRange
                                ...
                                onChange={(item) => {
                                    ...
                                    dispatch(
                                        {
                                            type: 'NEW_SEARCH',
                                            payload: { city, dates: [item.selection], options },
                                        },
                    ...
                    <div className={style.search_item}>
                        ...
                        <div className={style.search_item_option}>
                            <span className={style.option_txt}>
                                Min price
                                {/* <small className={style.night_batch}>per night</small> */}
                            </span>
                            <input
                                type="number"
                        ...
                        <div className={style.search_item_option}>
                            <span className={style.option_txt}>Number of People</span>
                            <input
                                type="number"
                                min={1}
                                value={options.adult}
